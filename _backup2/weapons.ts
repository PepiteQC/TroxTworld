...
export function weaponHarvestRange(id: string): number {
  ...
  // Priorité 1 : champ explicite sur le template (si présent)
  const explicit = (w as unknown as { harvestRange?: number }).harvestRange;
  if (typeof explicit === "number") return explicit;

  // Priorité 2 : heuristique par catégorie
  switch (w.category) {
    case "fusil_chasse":
    case "carabine":
    case "arc":
      return 4.6;      // armes de chasse longue portée
    case "poing":
      return 1.5;      // contact
    case "outil":
      return 2.5;      // hache, couteau…
    default:
      return 3.0;      // pistolet/fusil d'assaut, fallback
  }
}
// __WEAPONS_COMPAT_EXPORTS__
export function checkCarryLegality(_id: string, _zone?: string): { ok: boolean; reason?: string } {
  return { ok: true };
}
...
// __END_WEAPONS_COMPAT_EXPORTS__

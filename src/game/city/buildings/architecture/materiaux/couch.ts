...
const COUCH_DIMENSIONS: Record<CouchType, Partial<typeof DEFAULT_DIMENSIONS>> = {
  ...
  outdoor: { seatW: 1.8, seatD: 0.9, backH: 0.7, material: "fabric" as CouchMaterial }, // Matériau résistant
};
...
export class Couch {
  ...
  private buildStandardCouch(group: THREE.Group, D: typeof DEFAULT_DIMENSIONS): void {
    ...
    // Coussins
    this.cushions = [
      this.buildCushion(D, 0),
      this.buildCushion(D, 1),
      this.buildCushion(D, -1),
    ];
  ...
  private buildSectional(group: THREE.Group, D: typeof DEFAULT_DIMENSIONS): void {
    ...
    const chaiseBack = this.buildBack(D, 0.8, true);
    ...
    this.cushions.push(this.buildCushion(D, 0.5, true));
  ...
  private buildChaise(group: THREE.Group, D: typeof DEFAULT_DIMENSIONS): void {
    ...
    this.back = this.buildBack(D, D.seatW, true); // Dossier plus bas
    group.add(this.back);
  ...
  private buildOutdoorCouch(group: THREE.Group, D: typeof DEFAULT_DIMENSIONS): void {
    ...
    this.seat = this.buildSeat(D, undefined, weatherproofMaterial);
    group.add(this.seat);
    ...
    this.cushions = [
      this.buildCushion(D, 0, weatherproofMaterial),
      this.buildCushion(D, 0.5, weatherproofMaterial),
      this.buildCushion(D, -0.5, weatherproofMaterial),
    ];
    this.cushions.forEach(cushion => group.add(cushion));
  }

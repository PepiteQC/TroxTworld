...
import { computeLightState } from '../systems/LightManager'
...
function HotelSign({ position }: { position: [number, number, number] }) {
  ...
  useFrame(() => {
    ...
    const ls = computeLightState(time.hourDecimal, weather.currentConfig)
    if (signLightRef.current) {
      signLightRef.current.intensity = ls.signsOn ? 3.5 : 0
    }
    if (neonTextRef.current) {
      const mat = neonTextRef.current.material as THREE.MeshStandardMaterial
      mat.emissiveIntensity = ls.signsOn ? 3.0 : 0.05
    }
  })
  ...
  return (
    <group position={position}>
      ...
      <pointLight ref={signLightRef} position={[0, 0.5, 1]} color="#ff1133" intensity={0} distance={10} decay={2} />
    </group>
  )
}
...
function Antenna({ position }: { position: [number, number, number] }) {
  ...
  useFrame(({ clock }) => {
    ...
    const blink = Math.sin(clock.getElapsedTime() * 4.5) > 0
    const active = ls.signsOn && blink
    
    if (lightRef.current) lightRef.current.intensity = active ? 1.5 : 0
    if (coreRef.current) {
      (coreRef.current.material as THREE.MeshStandardMaterial).emissiveIntensity = active ? 2 : 0
    }
  })
  ...
  return (
    <group position={position}>
      ...
      <pointLight ref={lightRef} position={[0, 2.1, 0]} color="#ff0000" intensity={0} distance={10} decay={2} />
    </group>
  )
}
...
function LobbyEntrance() {
  ...
  useFrame(() => {
    ...
    if (canopyLightRef.current) {
      canopyLightRef.current.intensity = ls.streetsOn ? 2.5 : 0
    }
  })
  ...
  return (
    <group position={[0, 0, BLDG_D / 2]}>
      ...
      <pointLight ref={canopyLightRef} position={[0, 3.1, 2.5]} color="#fff8e0" intensity={0} distance={12} decay={2} />
...
export function HotelBuildingAdvanced({
  position = [0, 0, 0],
  rotation = [0, 0, 0]
}: {
  ...
  const yardLightRef1 = useRef<THREE.PointLight>(null)
  const yardLightRef2 = useRef<THREE.PointLight>(null)

  useFrame(() => {
    const time = useTimeStore.getState()
    const weather = useWeatherStore.getState()
    const ls = computeLightState(time.hourDecimal, weather.currentConfig)
    if (yardLightRef1.current) yardLightRef1.current.intensity = ls.streetsOn ? 2.5 : 0
    if (yardLightRef2.current) yardLightRef2.current.intensity = ls.streetsOn ? 2.5 : 0
  })
  ...
  return (
    <group position={position} rotation={rotation}>
      ...
      <group>
        ...
        <mesh position={[-6, BLDG_TOTAL_H + 0.8, -3]}>
          <boxGeometry args={[2, 1.0, 2]} />
          <meshStandardMaterial color="#2c3e50" roughness={0.8} />
        </mesh>
        <mesh position={[5, BLDG_TOTAL_H + 0.6, -1]} castShadow>
      ...
      <group>
        ...
        {[-10, 10].map((x, idx) => (
          <group key={idx} position={[x, 0, 19]}>
            ...
            <pointLight
              ref={idx === 0 ? yardLightRef1 : yardLightRef2}
              position={[0, 6.8, -0.4]}
              color="#fff0d0"
              intensity={0}
              distance={15}

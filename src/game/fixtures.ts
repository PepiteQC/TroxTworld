/**
 * ═════════════════════════════════════════════════════════════════════════════
 * 🚪 FIXTURES QUALITÉ RP QUÉBÉCOIS (v3.0) — Portes sécurisées, Salle de bain, Fenêtres
 * Fichier: src/game/fixtures.ts
 * Architecture : Cache VRAM unifié via getGeo(), userData RP, PBR haute qualité.
 * ═════════════════════════════════════════════════════════════════════════════
 */
...
function addBox(
  ...
  const mesh = new THREE.Mesh(getGeo("box", { w, h, d }), mat);
...
function addCyl(
  ...
  const mesh = new THREE.Mesh(getGeo("cylinder", { r, r2: r, h, seg }), mat);
  mesh.position.set(x, y, z);
  mesh.rotation.set(rx, ry, rz);
  mesh.castShadow = true;
  mesh.receiveShadow = true;
...
function addTorus(
  ...
  const mesh = new THREE.Mesh(
    getGeo("torus", { r: radius, tube, seg }),
    mat
  );
  mesh.position.set(x, y, z);
  mesh.rotation.set(rx, ry, rz);
...
// ─── LECTEUR DE CARTE (STYLE HID / ADT / VERKADA) ────────────────────────────
...
// ─── PORTE SÉCURISÉE (STYLE PALIER DE POSTE POLICE / BANQUE) ────────────────
...
export function buildSecurityDoor(): THREE.Group {
  ...
  // ── Gâche de serrure (strike plate) ──
  addBox(g, 0.035, 0.16, 0.03, chrome, 0.485, 1.06, 0.085);

  // ── Capteur de porte (Alarme ADT) ──
  addBox(
    g,
    0.025,
    0.06,
...
// ─── SALLE DE BAIN COMPLÈTE (Toilette, Lavabo, Bain) ─────────────────────────
...
export function buildBathroomFixtures(): THREE.Group {
  ...
  const tile = tex.mat?.("betonMur", 1.6, 1.2, 0.78, 0.05) ?? matLib.get(0xd0d0c8, 0.85, 0.05);
  const towelMat = tex.cloth?.("laine", "laineNrm", 0.9, 0.9, 0.95, 0xd8d2c6, 0.18) ?? matLib.get(0xd8d2c6, 0.9, 0);
  ...
  addTorus(toilet, 0.17, 0.024, porcelain, 0, 0.53, 0.06, Math.PI / 2, 0, 0, 22);
  ...
  // Miroir (décalé de 0.005 pour éviter le Z-fighting avec le cadre)
  const mirror = addBox(
    sink,
    0.56,
    0.7,
...
// ─── FENÊTRE AVEC RIDEAUX (Style résidentiel québécois) ──────────────────────

export function buildWindowWithCurtains(width = 1.35, height = 1.55): THREE.Group {
  const g = new THREE.Group();
  g.name = "window-curtains";

  const frame = matLib.get(QC_PALETTE.porte ?? 0x5a3a28, 0.72);
  const innerFrame = matLib.get(0x3d2b1f, 0.6, 0.05);
  const clothMat = tex.cloth?.("laine", "laineNrm", 1.4, 1.2, 0.9, 0x6a3a3a, 0.4) ?? matLib.get(0x6a3a3a, 0.9);
  ...
  const glass = new THREE.Mesh(
    getGeo("box", { w: width - 0.06, h: height - 0.08, d: 0.016 }),
  ...
  // ── Tringle à rideaux en laiton ──
  addCyl(g, 0.018, width + 0.24, rodMat, 0, height + 0.12, 0.12, 12, 0, 0, Math.PI / 2);

  // Embouts de tringle
  addCyl(g, 0.03, 0.03, rodMat, -(width + 0.24) / 2, height + 0.12, 0.12, 10, 0, 0, Math.PI / 2);
  addCyl(g, 0.03, 0.03, rodMat, (width + 0.24) / 2, height + 0.12, 0.12, 10, 0, 0, Math.PI / 2);
  ...
  for (const side of [-1, 1] as const) {
    ...
    // 3 plis pour donner du volume et de la profondeur au tissu
    for (let i = 0; i < 3; i++) {
      const fold = addBox(
        curtain,
        panelW,
...
// ─── EXPORTS ALIAS POUR COMPATIBILITÉ EXTERNE ────────────────────────────────

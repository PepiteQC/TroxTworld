...
export type DoorLockType = "standard" | "reinforced" | "electronic_pin" | "smart_keycard";
...
export class SwingDoor {
  ...
  // Paramètres de sécurité Québécois
  propertyId: string;
  slotId: string;
  lockType: DoorLockType = "standard";
  isLocked = true;
  isBreached = false;
  brokenState = 0;
  isHydroPowered = false;
  ...
  hit(impulse: number, attackerId?: string) {
    if (this.isLocked && !this.isBreached) {
      const resistanceFactor = this.lockType === "reinforced" ? 0.4 : 1.0;
      this.brokenState += Math.abs(impulse) * resistanceFactor;
      
      if (this.brokenState >= 100) {
        this.breachDoor(attackerId);
      } else {
        netEmit("door:rattle", { propertyId: this.propertyId, slotId: this.slotId, damage: this.brokenState });
        return;
      }
    }
  ...
  push(dt: number, force: number) {
    if (this.isLocked && !this.isBreached) return;
    this.vel += this.sign * Math.abs(force) * dt;
    this.latch = Math.max(this.latch, 2.6);
  }

  public breachDoor(breakerId?: string) {
    this.isBreached = true;
    this.isLocked = false;
    this.brokenState = 100;
    this.vel += this.sign * 15;
    
    this.leaf.rotation.z = 0.12 * this.sign;
    this.leaf.rotation.x = 0.08;

    if (breakerId) {
      addWantedPoints(breakerId, 30, "Méfait lourd et intrusion par effraction");
    }

    netEmit("door:breached", { propertyId: this.propertyId, slotId: this.slotId, breakerId });
  }

  public repairAndLock(lockType: DoorLockType = "standard") {
    ...
    this.leaf.rotation.z = 0;
    this.leaf.rotation.x = 0;

    netEmit("door:repaired", { propertyId: this.propertyId, slotId: this.slotId, lockType });
  }
  ...
  private playSlamSound() {
    if (Math.abs(this.vel) > 1.5) {
      netEmit("door:slam", { propertyId: this.propertyId, slotId: this.slotId });
    }
  }

  private syncNetworkPhysics() {
    const now = Date.now();
    if (now - this.lastNetworkSync > 150) {
      netEmit("door:physics_sync", {
...
export function buildGlassLeaf(
  ...
  const sticker = box(0.12, 0.12, 0.002, matLib.getEmissive(0x2563eb, 0x2563eb, 0.2), cx - 0.1, height * 0.5, 0.015);
  leaf.add(sticker);

  hinge.add(leaf);
  return {
    hinge,
    door: new SwingDoor(hinge, leaf, handle, swingSign, geomSign, width, opts?.propertyId, opts?.slotId),
  };
}
...
export function buildHouseFrontDoor(width = 0.95, height = 2.08): THREE.Group {
  ...
  const keypad = buildKeypadReader(width / 2 + 0.12, height * 0.5, 0.08);
  g.add(keypad);

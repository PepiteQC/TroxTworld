...
export function liveCharacterSnapshot() {
  const store = useGameStore.getState();
  const id = rpNet.selfId || "local";
  const job = jobById(store.rpJob);
  const now = Date.now();
  ...
  return {
    character: {
      id,
      userId: id,
      name: store.appearance.name || "Citoyen",
      gender: "male",
      build: "normal",
      skinTone: store.appearance.skin,
      hairStyle: store.appearance.hairStyle,
      hairColor: store.appearance.hair,
      outfitTop: store.appearance.outfit,
      outfitBottom: store.appearance.outfit,
      shoes: "bottes",
      job: store.rpJob,
      cash: store.cash,
      bank: store.bank,
      wanted: store.wantedStars,
      updatedAt: now,
    },
    inventory: Object.entries(store.inventory).map(([itemId, quantity]) => ({
      id: `${id}-${itemId}`,
      characterId: id,
      itemId,
      itemType: "item",
    ...
    vehicles: store.ownedVehicles.map((vehicleType: string) => ({
      id: `${id}-${vehicleType}`,
      characterId: id,
      vehicleType,
      licensePlate: "PNF-138",
      color: "rang",
    })),
    employment: {
      id: `${id}-job`,
    ...
    logs: store.ledger.slice(0, 20).map((e: { id: string; amount: number; label: string; at: number }) => ({
      id: e.id,
      characterId: id,
      type: e.amount >= 0 ? "credit" : "debit",
      message: `${e.label} · ${e.amount >= 0 ? "+" : ""}${e.amount}\u00a0$`,
      timestamp: e.at,
    })),
  };
}

/**
 * ═══════════════════════════════════════════════════════════════════
 * 🚗 TROXTWORLD / ETHERWORLD — CATALOGUE DES VÉHICULES (/api/vehicles/)
 * ═══════════════════════════════════════════════════════════════════
 * 
 ...
 *  - 📋 Immatriculation SAAQ, Plaquage & Taxes de vente
 * ═══════════════════════════════════════════════════════════════════
 */
...
export type VehicleCategory = "civilian" | "pickup_suv" | "police" | "emergency" | "heavy_truck" | "winter_mtq" | "motorcycle";
...
const VEHICLES_CATALOG: VehicleCatalogItem[] = [
...
async function handleServerRequest({ request }: { request: Request }): Promise<Response> {
  ...
  try {
    ...
    if (method === "POST") {
      ...
      if (!action || !vehicleId) {
        return new Response(JSON.stringify({ ok: false, error: "missing_vehicle_or_action" }), { status: 400, headers });
      }

      const veh = VEHICLES_CATALOG.find((v) => v.id === vehicleId);
      if (!veh) {
        return new Response(JSON.stringify({ ok: false, error: "vehicle_not_found" }), { status: 404, headers });
      }

      const nowTs = Date.now();
      const saaqPlate = `${String.fromCharCode(65 + Math.floor(Math.random() * 26))}${Math.floor(10 + Math.random() * 90)} ${String.fromCharCode(65 + Math.floor(Math.random() * 26))}${String.fromCharCode(65 + Math.floor(Math.random() * 26))}${String.fromCharCode(65 + Math.floor(Math.random() * 26))}`;
      const vinNumber = `2FTNE1E${Math.floor(10000000 + Math.random() * 90000000)}`;

      const tvqTpsTax = Math.round(veh.price * 0.14975);
      const saaqRegFee = 245.00;
